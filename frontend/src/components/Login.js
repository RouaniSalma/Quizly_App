...
const Login = () => {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      setForgotPasswordMessage(error.response?.data?.detail || 'Failed to send reset link');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="modal-overlay">
          <div className="forgot-password-modal">
            ...
            {forgotPasswordMessage && (
              <p className={`forgot-password-message ${forgotPasswordMessage.includes('sent') ? 'success' : 'error'}`}>
                {forgotPasswordMessage}
              </p>
            )}
            <button 
